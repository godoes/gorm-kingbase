...
// CopyIn 创建一个可用Tx.Prepare()处理的'COPY FROM'预备语句
...
// CopyInSchema 创建一个可用Tx.Prepare()处理的'COPY FROM'预备语句
...
func (ci *copyin) Query(_ []driver.Value) (r driver.Rows, err error) {
	err = ErrNotSupported
	r = nil
	return
}

// Exec 向COPY流中异步地插入数据，并返回先前调用相同COPY stmt时的错误
//
