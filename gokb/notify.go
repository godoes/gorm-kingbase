...
// NewListenerConn 创建一个新的ListenerConn
...
// ExecSimpleQuery 在连接上执行一个“简单查询”（即一个没有参数的查询）。可能的返回值为：
...
// NewDialListener 类似NewListener,但需要一个Dialer
...
// Ping 远程服务器以确保它还存在。Non-nil返回值意味着没有活跃的连接。
