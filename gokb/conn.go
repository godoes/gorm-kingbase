...
import (
	...
	"github.com/godoes/gorm-kingbase/gokb/oid"
	"github.com/godoes/gorm-kingbase/gokb/oid/mysqlOid"
	"github.com/godoes/gorm-kingbase/gokb/oid/oracleOid"
	"github.com/godoes/gorm-kingbase/gokb/oid/sqlserverOid"
	"github.com/godoes/gorm-kingbase/gokb/scram"
	"github.com/godoes/gorm-kingbase/gokb/sm3"
	"github.com/golang-sql/civil"
	"github.com/shopspring/decimal"
)

// Open 打开一个到数据库的新连接，name为连接串
...
// DialOpen 通过dialer打开一个到数据库的新连接
func DialOpen(_ Dialer, dsn string) (dc driver.Conn, err error) {
...
func getPrimary(cn *conn) (isPrimary bool, err error) {
	...
	for {
		...
		case 'D':
			...
			case []uint8:
				transactionReadOnly = strings.ToLower(string(dest))
			case string:
				transactionReadOnly = strings.ToLower(dest)
			default:
	...
	if transactionReadOnly == "off" {
		isPrimary = true
	} else if transactionReadOnly == "on" {
		isPrimary = false
	} else {
		errorf("unexpected value for transaction_read_only: %v", transactionReadOnly)
	}
...
// ParseOutValues 解析D报文中的结果集，将解析后的结果集赋值给应用的OUT类型变量
		...
		if sBind[i].isOut {
			...
			case cn.allOid.T_varcharbyte:
				...
				case *string:
					*dest.(*string) = string(byteValues[outNum])
				case *VarChar:
					*dest.(*VarChar) = VarChar(byteValues[outNum])
				case *VarCharMax:
					*dest.(*VarCharMax) = VarCharMax(byteValues[outNum])
				}
			case cn.allOid.T_nvarchar:
				...
				case *string:
					*dest.(*string) = string(byteValues[outNum])
				case *NVarCharMax:
					*dest.(*NVarCharMax) = NVarCharMax(byteValues[outNum])
				case sql.NullString:
					if paramLength[outNum] == -1 {
						(*dest.(*sql.NullString)).Valid = false
					} else {
			...
			case cn.allOid.T_nchar:
				...
				case *string:
					*dest.(*string) = string(byteValues[outNum])
				case *NChar:
					*dest.(*NChar) = NChar(byteValues[outNum])
				}
...
// QuoteIdentifier 将标识引用起来作为SQL语句的一部分(比如一个表或一个列名):
//
//	tblname := "my_table"
//	data := "my_data"
...
// QuoteLiteral 将字符引起来作为SQL的一部分(经常被用来向DDL和其它不接收参数的语句传字符的参数):
//
