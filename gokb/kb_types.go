...
// Dialer 为dialer接口，与创建网络连接相关
type Dialer interface {
	Dial(network, address string) (net.Conn, error)
	DialTimeout(network, address string, timeout time.Duration) (net.Conn, error)
}

// DialerContext 为需要上下文的dialer接口
...
// ByteaArray 表示一维的bytea类型的数组
type ByteaArray [][]byte

// Float64Array 表示一维的双精度数组
type Float64Array []float64

// ArrayDelimiter 可由driver.Valuer或sql.Scanner实现
// 通过GenericArray来重写数组分隔符
type ArrayDelimiter interface {
	// ArrayDelimiter 返回当前元素类型的分隔符
	ArrayDelimiter() string
}

// BoolArray 表示一维的布尔类型的数组
type BoolArray []bool

// GenericArray 实现了driver.Valuer和sql.Scanner接口，作为任何维数的数组或切片
type GenericArray struct{ A interface{} }

// Int64Array 表示一维的整型数组
type Int64Array []int64

// StringArray 表示一维的字符串类型的数组
type StringArray []string

/*****************notify.go*****************/

// Notification 表示来自数据库的一个notification
...
// ErrChannelAlreadyOpen 在通道打开后从Listen返回
var ErrChannelAlreadyOpen = errors.New("kb: channel is already open")

// ErrChannelNotOpen 在通道未打开时从Unlisten返回
var ErrChannelNotOpen = errors.New("kb: channel is not open")

// ListenerEventType 为监听器时间类型的枚举
type ListenerEventType int

const (
	// ListenerEventConnected 在数据库连接初始化时发出
	// 错误消息总是为空
	ListenerEventConnected ListenerEventType = iota

	// ListenerEventDisconnected 在出错或调用Close导致数据库连接丢失时发出
	// 错误消息为丢失连接的原因
	ListenerEventDisconnected

	// ListenerEventReconnected 在数据库连接丢失并重建后发出
	// 错误信息总是为空
	// 在此时间被发送后，一个空的kb.Notification将被发送到Listener.Notify通道
	ListenerEventReconnected

	// ListenerEventConnectionAttemptFailed 在尝试连接数据库但失败后被发送
	// 错误信息为连接没有成功的原因
	ListenerEventConnectionAttemptFailed
)

// EventCallbackType 为事件回调类型
type EventCallbackType func(event ListenerEventType, err error)

// Listener 为监听数据库的notifications提供了一个接口
...
// Connector 结构体表示gokb驱动的固定配置，实现database/sql/driver的Connector接口
...
// NullTime 表示一个可能为空的time.Time类型的值
...
// ErrorCode 为5个字符长的错误码
type ErrorCode string

type ErrorClass string

// KBError 为支持旧版本的接口，新版本直接使用Error即可
...
// ReturnStatus 兼容mssql的ReturnStatus，用于接收存储过程的返回值
