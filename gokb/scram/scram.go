...
// Client SCRAM相关的认证方法(如SCRAM-SHA-1, SCRAM-SHA-256等).
...
// NewClient 返回一个包含提供的哈希算法的Client结构体
//
...
// Step 处理来自服务器的传入数据，并通过Client.Out获取接下来所需的数据
...
func (client *Client) step1(_ []byte) (err error) {
	...
	_, _ = escaper.WriteString(&client.authMsg, client.user)
...
func (client *Client) clientProof() (bytes []byte) {
	...
	clientKey, newHash := mac.Sum(nil), client.newHash()
	newHash.Write(clientKey)
	storedKey := newHash.Sum(nil)
