...
func sslClientCertificates(tlsConf *tls.Config, o values) (err error) {
	...
	if len(sslcert) == 0 && currentUser != nil {
		sslcert = filepath.Join(currentUser.HomeDir, ".kingbase", "kingbase.crt")
	}
	if len(sslcert) == 0 {
		return nil
	}
	...
	sslkey := o["sslkey"]
	if 0 == len(sslkey) && nil != currentUser {
		sslkey = filepath.Join(currentUser.HomeDir, ".kingbase", "kingbase.key")
	}
